...
-- Modified so that certain weapons can destroy glass targets.
...
local function _defaultDamageCallback(system, target, amount, damageType, dealer, hitInfo, damageData)
	if target:IsA("Humanoid") then
		if not WeaponsSystem.getPlayerFromHumanoid(target) then -- Don't kill other players.
		...
		TargetMissedEvent:FireClient(dealer)
	elseif target.Name == "GemTarget" then
		local tool = dealer.Character:FindFirstChildOfClass("Tool")
		if tool:GetAttribute("CanBreakGlass") then
			GemTargetHitEvent:FireClient(dealer, dealer, target)
		end
	elseif target.Name == "GlassTarget" then
		local tool = dealer.Character:FindFirstChildOfClass("Tool")
		if tool:GetAttribute("CanBreakGlass") then
			GlassTargetHitEvent:FireClient(dealer, dealer, target)
		end
	else
		TargetMissedEvent:FireClient(dealer)
	end
